...
export function addTodoToProject() {
    ...
    priorityBox.addEventListener('click', function (e) {
        ...
        switch (target) {
            case "prior3":
                document.querySelector("#prior3").
                priority = 'High';
                break;
            case "prior2":
                document.querySelector("#prior2").
                priority = 'medium';
                break;
            default:
                document.querySelector("#prior1").
                priority = 'Low'
                break;
        }
    })
    todoDialog.addEventListener('click', function (event) {
        ...
        if (target.className === "cancelBtn") closeDialog(todoDialog);
            ...
            const todo = new Todo(todoNameInput.value, todoDesc.value, todoDate.value, priority);
            console.log(todo);
